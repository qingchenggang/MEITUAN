<template>
  <div id="meishi-detail">
    ...
    <div v-for="(list,index) in lists" :key="index" v-if="$route.params.lastid==list.id">
      <div class="hearder">
...
<script>
  ...
  export default {
    ...
    async mounted(){
      ...
      let ha= JSON.parse(localStorage.getItem('last'))
      if(ha==null){
        this.lists=this.lists
      }else{
        this.lists = JSON.parse(localStorage.getItem('last'))
        if(this.lists[this.$route.params.lastid-1].kong==true){
          this.lists[this.$route.params.lastid-1].kong=true
        }else if(this.lists[this.$route.params.lastid-1].kong==false){
          this.lists[this.$route.params.lastid-1].kong=false
        }
      }
    },
    methods:{
      ...
      start(state,id){
        ...
        if(this.lists[id-1].kong==true){
          ...
          localStorage.setItem('last', JSON.stringify(this.lists));
          zhi=JSON.parse(localStorage.getItem('zhuangtai3'));
          if(zhi!=null ){
         //  alert('第一个')
            this.localData.push(...zhi,this.lists[id-1])
            localStorage.setItem('zhuangtai3', JSON.stringify(this.localData));
           // console.log(this.localData)
          }else{
          // alert('第二个')
           if(this.dsdData.length==0){
              this.dsdData.push(this.lists[id-1])
              localStorage.setItem('zhuangtai3', JSON.stringify(this.dsdData));
            }
          }
          setTimeout(()=>{
            this.success=false
          ...
          localStorage.setItem('last', JSON.stringify(this.lists));
          this.newLists=JSON.parse(localStorage.getItem('zhuangtai3'));
         // console.log('取消之前的数组====>',this.newLists)
          this.newLists=this.newLists.filter(item=>{
            return item.id!=id
          })
        //  console.log('取消之后的数组====>',this.newLists)
          localStorage.setItem('zhuangtai3', JSON.stringify(this.newLists));
          this.newLists = []
          this.localData = []
          setTimeout(()=>{
            this.fail=false
